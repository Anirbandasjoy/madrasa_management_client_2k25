...
export default function ExNavbar() {
  ...
  const navClass = (path: string) =>
    `text-xl lg:text-lg text-normal transform duration-500 hover:underline hover:text-customDark ${path === pathname ? "text-success" : "text-neutral"
    }`;
  const navList = (
    <>
      ...
      <Link href="/profiles" className={navClass("/contact")}>
        contact
      </Link>
    </>
  );
